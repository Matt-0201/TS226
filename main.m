...
%% Execution simultanée des codes convolutifs
% polynomes = {[2 3], [5 7], [13 15], [133 171]};
% noms_codages = {'(2,3)_8','(5,7)_8','(13,15)_8','(133,171)_8'};
% contraintes = [2 3 4 7];
% 
% colors = {'r','b','g','k'}; % couleurs pour les codes
% 
% trellises = cell(size(polynomes));
% for k = 1:length(polynomes)
%     trellises{k} = poly2trellis(contraintes(k), polynomes{k});
% end

%% Execution simultanée des codes convolutifs récusifs
polynomes = {[5 7], [5 7], [13 15], [13 15]};
feedback = {[], 7, [], 15};
noms_codages = {'(5,7)_8','(1,5/7)_8','(13,15)_8','(1,13/15)_8'};
contraintes = [3 3 4 4];
...
for k = 1:length(polynomes)
    if mod(k, 2) == 0
        trellises{k} = poly2trellis(contraintes(k), polynomes{k},feedback{k});
    else
        trellises{k} = poly2trellis(contraintes(k), polynomes{k});
    end
    %trellises{k} = poly2trellis(contraintes(k), polynomes{k},feedback{k});
...
%% Simulation
for indice_code = 1:length(trellises)
    trellis = trellises{indice_code};
    fprintf('Simulation pour le code %s\n', noms_codages{indice_code});
    
    %Initialisation vecteurs de resultats
    TEP = zeros(1,length(EbN0dB));
    TEB = zeros(1,length(EbN0dB));
    
    %Créaation des courbes
    hTEB = semilogy(EbN0dB, TEB, 'LineWidth',1.5,'XDataSource','EbN0dB','YDataSource','TEB');
    hTEP = semilogy(EbN0dB, TEP, 'LineWidth',1.5,'XDataSource','EbN0dB','YDataSource','TEP');
    
    %Definition couleur + titre
    set(hTEB, 'Color', colors{indice_code}, 'DisplayName', ['TEB ' noms_codages{indice_code}]);
    set(hTEP, 'Color', colors{indice_code}, 'DisplayName', ['TEP ' noms_codages{indice_code}]);
    for iSNR = 1:length(EbN0dB)
        ...
        while (bitErr < nbrErreur && pqtNbr*K < nbrBitMax)
            ...
            u_rec = viterbi_decode(Lc,trellis); %L'utilisation de LC à la place de y serait plus optimale
